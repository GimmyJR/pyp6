...
const app = new Hono()
  .get(
    ...
    async (c) => {
      ...
      if (user) {
        ...
        votedPostIds = votedPosts.map((vote: any) => vote.postId);
      ...
      const fetchMultiplier = 3; // Define the missing variable
      ...
      const [candidatePosts, total] = await Promise.all([
        db.post.findMany({
          ...
          orderBy: [
            ...
            { averageRating: 'desc' }, // Higher rated content
          ],
        }),
      ...
      const newlySeenPostIds = finalPosts.map((post: any) => post.id);
  ...
  .get('/top-creators-today', async (c) => {
    ...
    const postsWithScores = todayPosts.map((post: any) => {
      // Use vote weights from the voting algorithm
      const weightedVotes = post.vote.reduce(
        (acc: any, vote: any) => acc + vote.rating * vote.weight,
        0
      );

      const totalVoteWeight =
        post.vote.reduce((acc: any, vote: any) => acc + vote.weight, 0) || 1; // Avoid division by zero
    ...
    if (topPosts.length < 3) {
      ...
      const existingPostIds = topPosts.map((post: any) => post.id);
      ...
      const recentPostsWithScores = recentPosts.map((post: any) => {
        // Use vote weights from the voting algorithm
        const weightedVotes = post.vote.reduce(
          (acc: any, vote: any) => acc + vote.rating * vote.weight,
          0
        ...
        const totalVoteWeight =
          post.vote.reduce((acc: any, vote: any) => acc + vote.weight, 0) || 1;
      ...
      const formattedPosts = combinedTopPosts.map((post: any) => ({
        id: post.id,
        caption: post.caption,
        image: post.image,
        creator: post.creator,
